<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<Odp.InteractiveSdk.Mvc.Models.Rating.RateInfo>" %>
<%@ Import Namespace="Odp.InteractiveSdk.Mvc" %>

	<%
		if (Model.ReadOnly || Model.HasUserVoted(Request))
		{
            Html.RenderPartial("RatesPlusMinus", new Odp.InteractiveSdk.Mvc.Models.Rating.VoteResults() { Negative = this.Model.NegativeRates, Positive = this.Model.PositiveRates });
		}
		else
		{
	%>
...
<script type="text/javascript">
	...
	function ShowRecaptcha() {
	    Recaptcha.create('<%= System.Configuration.ConfigurationManager.AppSettings["RecaptchaPupblicKey"]%>', "RecaptchaDiv", { callback: Recaptcha.focus_response_field });
