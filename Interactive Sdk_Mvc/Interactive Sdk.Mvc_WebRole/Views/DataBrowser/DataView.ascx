...
<label style="display:none" id="labelError" title="<%= Html.Encode(ViewData.Model.DBErrorLine1) %>"></label>
    ...
    <td align="right">    
        ...
        <div id="eidCSVDownloadDiv">
            <%= Html.ActionLink("Download as CSV", "DownloadCsv", "DataBrowser",
                                  new
				...
				<tr class="<%= odd ? "co" : "ce" %>">
                    ...
                    <td style="white-space: nowrap; text-align: left">
                        <% if (first) { first = false; %>
							<img src='<%= UIConstants.DBPC_KeyImagePath %>' alt='<%= row[col].ToString() %>' title='<%= row[col].ToString() %>' longdesc='<%= UIConstants.DBPC_KeyLongDesc %>' style="height: 15px; border-width: 0px;" /> <%= row[col].ToString() %>
                        <% } else { %>
							<%= Object.ReferenceEquals(row[col], null) ? "&nbsp;" : Html.Encode(row[col]) %>
                        <% } %>
                    </td>
                    <% } %>
                </tr>
