...
namespace Odp.InteractiveSdk.Mvc.Models
{
	public sealed class DatasetListModel
	{
		...
		{
			{Field.Image, t => t.EntitySet.Image},
		...
		//sl-king
		//needed because each entityset has its own link to the OData datasource
		public EntitySet GetEntitySet(string container, string table)
		{
			IQueryable<EntitySet> ents = GetEntitySets(null).AsQueryable<EntitySet>();
			var list = from e in ents where e.Name == table && e.Source == container select e;
		...
		//    //  yield return set;
		//  }
		//  return list.AsEnumerable<EntitySet>();
		//}
		...
		{
			////for debugging LINQ below
			//var list = new List<EntitySetWrapper>();
			//var dis = DatasetInfos;
			//foreach(var es in filteredSets)
			//  foreach(var di in dis)
			//    if(Helper.GenerateDatasetItemKey(es.ContainerAlias, es.EntitySetName) == di.RowKey)
			//      list.Add(new EntitySetWrapper(){EntitySet = es, PositiveVotes = di.PositiveVotes, NegativeVotes = di.NegativeVotes, Views = di.views_total});
