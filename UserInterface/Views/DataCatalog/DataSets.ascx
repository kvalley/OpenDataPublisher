...
<%@ Control Language="C#" Inherits="ViewUserControl<DatasetListModel>" %>
<% 
    var odd = true;
...
%>
<tr class="<%= odd ? "co" : "ce" %>">
    <td style="text-align: center;"><a href="<%=link %>">
        <img src="<%= entity.EntitySet.Image %>" style="margin: 5px; height: 70px; width: 150px; border: 1px solid #282828;" /></a></td>
    <td><a href="<%=link%>"><b>
        <%=entity.EntitySet.Name%></b></a>
        <div class="description" style="padding-top: 10px;">
            <%=entity.EntitySet.Description%>
        </div>
    </td>
    <% 
       var downloadHTML = "";
       ...
       foreach (OrderedDictionary download in entity.EntitySet.DownloadLinks)
       {
           ...
           {
               downloadHTML = "<a href=\"" + download["Link"].ToString() + "\"><img src=\"" + download["IconLink"] + "\"></a>";
           }
           else if (download["Type"].ToString() == "KML")
           {
               downloadKML = "<a href=\"" + download["Link"].ToString() + "\"><img src=\"" + download["IconLink"] + "\"></a>";
           }
           else if (download["Type"].ToString() == "RSS")
           {
               downloadRSS = "<a href=\"" + download["Link"].ToString() + "\"><img src=\"" + download["IconLink"] + "\"></a>";
           }
    ...
    <td class="downloadLink">
        <%= downloadRSS %></td>
    <td class="downloadLink"><a href="<%=link%>">View More Formats</a>
